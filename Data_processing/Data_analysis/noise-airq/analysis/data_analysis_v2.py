...
def fmt_sig(x: float, sig: int = 3) -> str:
    if not np.isfinite(x):
        return "NA"
    # 3 significant digits, scientific if needed
    s = f"{x:.{sig}g}"
    return s
...
def run_pair(air: pd.DataFrame, noise: pd.DataFrame, air_kit: str, noise_kit: str,
    ...
    def scope_block(tag: str, dfin: pd.DataFrame):
        ...
        for air_var, alt in AIR_VARS_CORE.items():
            ...
            results_rows.append({
                ...
                "air_var": air_var, "alt": alt, "noise_var": best.get("noise_var",""),  # filled below
            ...
            # Fill noise_var for best (we need to know which list it came from)
            # Find which noise_var produced 'best' by identity
            found_noise = None
            for nvar, rows_fwd in forward_rows_by_noise.items():
                for rr in rows_fwd:
                    if (rr["lag_min"] == best["lag_min"]
                        and rr["N"] == best["N"]
                        and abs(rr["r"] - best["r"]) < 1e-12
                        and abs(rr["p_adj"] - best["p_adj"]) < 1e-16):
                        found_noise = nvar; break
                if found_noise:
                    break
            if found_noise:
                results_rows[-1]["noise_var"] = found_noise

    # Two scopes: ALL and OCCUPIED
    scope_block("ALL", df)
    ...
    def bullets_for_scope(scope: str):
        ...
        for air_var, alt in AIR_VARS_CORE.items():
            ...
            if not row: 
                continue
            sign_arrow = "↑" if row["pearson_r"]>0 else "↓"
    ...
    md += [f"# Analysis Summary — {air_kit} × {noise_kit}",
           ...
           "",
           "### Key endpoints (ALL minutes)"] + bullets_for_scope("ALL") + ["",
           "### Key endpoints (OCCUPIED minutes)"] + bullets_for_scope("OCCUPIED") + ["",
